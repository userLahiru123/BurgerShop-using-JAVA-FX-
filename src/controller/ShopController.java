...
public class ShopController {
    ...
    public ObservableList<Burger> sort() {
        ...
        ObservableList<Burger> sortedlist = FXCollections.observableArrayList();
        sortedlist.addAll(sortBurgerArray);

        return sortedlist;
    }

    public ObservableList<Burger> searchCustomer(String customerId) {
        int count = 0;
        for (int i = 0; i < list.listSize(); i++) {
            if (list.get(i).getCustomerId().equals(customerId)) {
                count++;
        ...
        ObservableList<Burger> customerList = FXCollections.observableArrayList();
        customerList.addAll(array);
        return customerList;
    }

    public ObservableList<Burger> viewOrders(int status) {
        int count = 0;
        for (int i = 0; i < list.listSize(); i++) {
        ...
        ObservableList<Burger> deliveredList = FXCollections.observableArrayList();
        deliveredList.addAll(deliveredOrderArray);
    ...
    public static ShopController getInstance(){
        if(instance == null) return instance = new ShopController();
        return instance;
    }
